...
interface SmsSendersToolbarProps {
  search: string;
  onSearchChange: (value: string) => void;
}

export function SmsSendersToolbar({
  search,
  onSearchChange,
}: SmsSendersToolbarProps) {
  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-center sm:justify-between">
      <div className="relative w-full min-w-0 sm:max-w-xs sm:flex-1">
        ...
        <Input
          type="search"
          placeholder={t("smsSenders.search")}
          value={search}
          onChange={(e) => onSearchChange(e.target.value)}
          className="pl-8"
          aria-label={t("smsSenders.search")}
        />
      </div>
      <Button variant="outline" className="shrink-0 text-muted-foreground" asChild>
        <Link to="/bank-sms/sms-senders/create">
